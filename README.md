...
6. Compile to executable with `clang++ -std=c++20 nm_cpp.cpp -o nm_cpp` or `g++ -std=c++20 nm_cpp.cpp -o nm_cpp`. Compile to WASM wih WASI support with `/path/to/wasi-sdk/bin/clang++ -std=c++2a --wasm-opt -O3 -flto -fno-exceptions nm_cpp.cpp -o nm_cpp_wasi.wasm`.
